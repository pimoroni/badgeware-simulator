...
extern "C" {
  ...
  MPY_BIND_STATICMETHOD_VAR(4, pattern, {
    ...
    if(!mp_obj_is_type(args[2], &type_color)) {
      mp_raise_TypeError(MP_ERROR_TEXT("parameter must be of color type"));
    }

    const image_obj_t *target = (image_obj_t *)MP_OBJ_TO_PTR(args[0]);
    const color_obj_t *c1 = (color_obj_t *)MP_OBJ_TO_PTR(args[1]);
    const color_obj_t *c2 = (color_obj_t *)MP_OBJ_TO_PTR(args[2]);

    brush_obj_t *brush = mp_obj_malloc(brush_obj_t, &type_brush);
    if(mp_obj_is_int(args[3])) {
      // brush index supplied, use pre-baked brush
      int i = mp_obj_get_int(args[3]);

      if(i < 0 || i > 37) {
        mp_raise_TypeError(MP_ERROR_TEXT("pattern index must be a number between 0 and 37"));
      }

      brush->brush = m_new_class(pattern_brush_t, target->image, c1->c, c2->c, i);

    }else if(mp_obj_is_type(args[3], &mp_type_tuple)) {
      size_t len;
      mp_obj_t *items;
      ...
      brush->brush = m_new_class(pattern_brush_t, target->image, c1->c, c2->c, p);
    } else {
...
;
  ...
  MPY_BIND_STATICMETHOD_VAR(2, image, {
    if(!mp_obj_is_type(args[0], &type_image)) {
      mp_raise_TypeError(MP_ERROR_TEXT("parameter must be of image type"));
    }
    ...
    if(n_args == 2) {
      ...
      if(!mp_obj_is_type(args[2], &type_mat3)) {
        mp_raise_TypeError(MP_ERROR_TEXT("parameter must be of matrix type"));
      }

      mat3_obj_t *transform = (mat3_obj_t *)MP_OBJ_TO_PTR(args[2]);
