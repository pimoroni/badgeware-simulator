...
def draw_graph(title, graph, x, y, bar_width=8, bar_height=100, max_value=None):
    ...
    debug.shape(shape.line(0, y, debug.width, y, 2))
    ...
    if graph:
        ...
        for avg in graph:
            ...
            debug.shape(shape.rectangle(x, y + bar_height - avg, bar_width, avg))
            x += bar_width + 1
    debug.shape(shape.line(0, y + bar_height, debug.width, y + bar_height, 2))
